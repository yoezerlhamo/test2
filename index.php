...
// Fetch total income
$total_income_query = mysqli_query($con, "SELECT SUM(amount) as total_income FROM budget WHERE user_id = '$userid' AND category='income'");
if (!$total_income_query) {
    die("Database query failed: " . mysqli_error($con));
}
$total_income_row = mysqli_fetch_assoc($total_income_query);
$total_income = $total_income_row['total_income'] ? $total_income_row['total_income'] : 0; // Default to 0 if null

// Fetch total expenses
$total_expenses_query = mysqli_query($con, "SELECT SUM(amount) as total_expense FROM budget WHERE user_id = '$userid' AND category='expense'");
if (!$total_expenses_query) {
    die("Database query failed: " . mysqli_error($con));
}
$total_expenses_row = mysqli_fetch_assoc($total_expenses_query);
$total_expenses = $total_expenses_row['total_expense'] ? $total_expenses_row['total_expense'] : 0; // Default to 0 if null

$notification_message = '';
$notification_class = ''; // either 'success' or 'error'

if ($total_expenses > $total_income) {
    $notification_message = "Your expenses exceed the total budget of Nu. {$total_income}!";
    $notification_class = 'danger'; // Use 'danger' for error messages
} else {
    $notification_message = "Your budget is within limits.";
...
<!-- Notification Alert -->
<div class="alert alert-<?php echo $notification_class; ?>" role="alert" id="notification">
